...
function scatterPlot3d( parent )
{
  ...
  function initializePlot() {
    initializeAxis(0, [0, 80]);
    initializeAxis(1, [0, 8]);
    initializeAxis(2, [0, 300]);
  }

  function initializeAxis( axisIndex, domain )
  {
    var key = axisKeys[axisIndex];
    drawAxis( axisIndex, key, initialDuration, domain );
  ...
  function drawAxis( axisIndex, key, duration, domain ) {

    var scale = d3.scale.linear()
      .domain( domain ) // demo data range
      .range( axisRange )
  ...
  function initializeDataGrid(data) {
    ...
    while (firstRow[index]) {
      var plant = firstRow[index].split('/');
      /* TODO: use 4th data point */
      rows.push({x: plant[0], y: plant[1], z:plant[2]});
      index++;
    }
  ...
  var rowIndex = 1;
  var data;
  client.open('GET', './history.csv');
  client.onreadystatechange = function() {
    csv = client.responseText;
    data = d3.csv.parseRows(csv);

    rows = initializeDataGrid(data);
